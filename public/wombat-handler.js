(function () {
  try {
    ...
    if (/^(?:http|ws)s?:\/[^/]/i.test(dest_fullurl)) {
      ...
      if (fixedDest !== dest_fullurl) {
        ...
        try { window.history.replaceState(null, "", newPath); } catch (_) {}
    ...
    function normalizeProxyUrl(u) {
      if (!u) return u;
      var s;
      try { s = u.toString(); } catch (_) { return u; }
    ...
    if (window && window._WBWombat && !window._wb_wombat) {
      ...
      (function patchWebSocket() {
        try {
          ...
          window.WebSocket = function (url, protocols) {
            ...
            try { url = rewriteIfPossible(url); } catch (_) {}
      ...
      function scheduleSave() {
        ...
        saveTimer = setTimeout(function () {
          saveTimer = -1;
          try {
            realSetItem(storeKey, JSON.stringify({ data: hostLocal, keyOrder: keyOrder }));
          } catch (_) {}
        }, 50);
      }

      function indexOfKey(k) {
        for (var i = 0; i < keyOrder.length; i++) if (keyOrder[i] === k) return i;
        return -1;
      }
      ...
      Object.defineProperty(ls, "length", {
        configurable: true,
        enumerable: false,
        get: function () { return keyOrder.length; }
      });
      ...
      (function patchBlob() {
        try {
          ...
          try { Object.defineProperty(BlobWrapper, "name", { value: "Blob" }); } catch(_) {}
      ...
      (function patchWorkerRewriter() {
        try {
          ...
          window._wb_wombat.rewriteWorker = function (workerUrl) {
            ...
            // Normalize and rewrite; avoid blocking sync XHR
            try {
              workerUrl = normalizeProxyUrl(workerUrl);
              workerUrl = rewriteIfPossible(workerUrl);
            } catch (_) {}

            // Create a small loader that imports the actual script inside the worker
            try {
              var loader = "try{importScripts(" + JSON.stringify(workerUrl) + ")}catch(e){setTimeout(function(){throw e})}";
              var blobUrl = window.URL.createObjectURL(new Blob([loader], { type: "application/javascript" }));
              return this._womginx_rewriteWorker(blobUrl);
            } catch (_) {
              // Fallback to original mechanism if any error
      ...
      function initialRewrite() {
        try {
          ...
          for (var i = 0; i < nodes.length; i++) {
            var el = nodes[i];
            // Avoid duplicating scripts: do not clone, just rewrite
            // If some sites need the "clone trick", ensure original is removed; default is rewrite only.
            rewriteAndMark(el);
          }
        } catch (_) {}
      }
      ...
      var currentLocationProp = {
        get ancestorOrigins() { _syncLocationObj(); return window.location.ancestorOrigins; },
 get href() { _syncLocationObj(); return locationObj.href; },
 set href(value) { window.location.href = window._wb_wombat.rewriteUrl(value); },
 get protocol() { _syncLocationObj(); return locationObj.protocol; },
 set protocol(value) { window.location.protocol = value; },
 get host() { _syncLocationObj(); return locationObj.host; },
 set host(value) { window.location.host = value; },
 get hostname() { _syncLocationObj(); return locationObj.hostname; },
 set hostname(value) { window.location.hostname = value; },
 get port() { _syncLocationObj(); return locationObj.port; },
 set port(value) { window.location.port = value; },
 get pathname() { _syncLocationObj(); return locationObj.pathname; },
 set pathname(value) { window.location.pathname = value; },
 get search() { _syncLocationObj(); return locationObj.search; },
 set search(value) { window.location.search = value; },
 get hash() { _syncLocationObj(); return locationObj.hash; },
 set hash(value) { window.location.hash = value; },
 get origin() { _syncLocationObj(); return locationObj.origin; },
 ...
 toString() { _syncLocationObj(); return locationObj.href; }
      };
      ...
      // Optional: neutralize service workers (many sites break behind a proxy due to SW scope/CORS)
      // Uncomment if you see breakage loops caused by SW.
      /*
       * (function patchServiceWorker() {
       *   try {
       *     if (!("serviceWorker" in navigator)) return;
       *     var sw = navigator.serviceWorker;
       *     if (sw.__womginx_patched__) return;
       *     sw.__womginx_patched__ = true;
       *     var noopReg = {
       *       then: function (cb) { try { cb(this); } catch(_) {} return this; },
       *       catch: function () { return this; }
    };
    sw.register = function () { return noopReg; };
    sw.getRegistrations && sw.getRegistrations().then(function (regs) {
    try { regs.forEach(function (r) { r.unregister && r.unregister(); }); } catch (_) {}
    });
    } catch (_) {}
    })();
*/
      ...
      if (document.readyState === "loading") {
        window.addEventListener("DOMContentLoaded", function () {
          try { initialRewrite(); observeMutations(); } catch (_) {}
        });
      } else {
        try { initialRewrite(); observeMutations(); } catch (_) {}
      }
